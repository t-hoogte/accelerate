...
transform :: Monad m  -- The interpreter will want to use this with `m ~ Identity`, but other backends might not (e.g. `m ~ CodeGen PTX`)
          => (forall body. op body -> ToIn aenv body -> m (ToOut aenv body)) -- function to handle primitives, might end up in the typeclass of `op` 
          -> ClusterAST aenv op i o -- Tree of fused primitives
          -> i                 -- input
          -> m o               -- output

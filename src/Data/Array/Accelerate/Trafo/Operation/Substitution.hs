...
reindexPartial :: (Applicative f) => ReindexPartial f env env' -> OperationAcc exe env t -> f (OperationAcc exe env' t)
reindexPartial k = reindexA' (ReindexF k)

reindexPartialAfun :: (Applicative f) => ReindexPartial f env env' -> OperationAfun exe env t -> f (OperationAfun exe env' t)
reindexPartialAfun k = reindexAfun' (ReindexF k)

instance Sink (OperationAcc exe) where
  weaken k = runIdentity . reindexPartial (Identity . (k >:>))

instance Sink (OperationAfun exe) where
...
reindexA' :: forall exe f env env' t. (Applicative f) => SunkReindexPartial f env env' -> PreOpenAcc (Execute exe) env t -> f (PreOpenAcc (Execute exe) env' t)
reindexA' k = \case
  ...
  where
    travA :: PreOpenAcc (Execute exe) env s -> f (PreOpenAcc (Execute exe) env' s)
    travA = reindexA' k

reindexAfun' :: (Applicative f) => SunkReindexPartial f env env' -> OperationAfun exe env t -> f (OperationAfun exe env' t)
...
pair :: forall exe env a b. OperationAcc exe env a -> OperationAcc exe env b -> OperationAcc exe env (a, b)
pair a b = goA weakenId a
  where
    ...
    --
    goA :: env :> env' -> OperationAcc exe env' a -> OperationAcc exe env' (a, b)
    ...
    goB :: GroundVars env' a -> OperationAcc exe env' b -> OperationAcc exe env' (a, b)
...
alet :: GLeftHandSide t env env' -> OperationAcc exe env t -> OperationAcc exe env' s -> OperationAcc exe env s
